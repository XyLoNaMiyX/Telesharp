...
namespace TLSharp.Core.Network
{
    public class MtProtoSender
    {
        ...
        private Tuple<byte[], long, int> DecodeMessage(byte[] body)
        {
            ...
            {
                long remoteAuthKeyId = inputReader.ReadInt64();
                byte[] msgKey = inputReader.ReadBytes(16);
        ...
        private bool HandleRpcResult(long messageId, int sequence, TBinaryReader messageReader, MTProtoRequest request)
        {
			...
			*/
            ...
            { // rpc_error
                ...
                else if (errorMessage.Equals("AUTH_KEY_UNREGISTERED")) // TODO handle better, atm delete session.dat
                {
